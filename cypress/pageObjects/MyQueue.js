...
export class MyQueue extends BasePage {
    ...
    clickQueueButton(eConsult, button, caption){
        cy.contains(eConsult).siblings('.container').first().get(button).contains(caption).click();
    }
    ...
    clickOnAttachment() {
        //to save time I just used the selector tha cypress tool provided, woult be ideal to save what was teh attachment that the user was working on and then access it again.
